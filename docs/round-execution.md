...
- Legacy orphan reconciliation requires exactly one valid A score and one valid B score. A round left with zero, one, or otherwise invalid scores is removed inside the claim transaction (including any pre-existing round usage), then the durable execution is claimed again so it cannot be charged or advanced half-written. If the legacy worker already advanced the session head to that partial round, recovery atomically rolls the head back one round and pauses non-terminal sessions before the engine derives its next target; later-round contradictions fail closed.
- Runtime startup repairs pre-existing duplicate `debate_round` usage rows by keeping the earliest `(sessionId, roundNumber)` row, then installs a partial unique index. Other usage event types remain independent. New reconciliation uses that constraint idempotently, and a database trigger rejects `debate_round` usage unless the matching round already has both judge scores; this closes the rolling-upgrade window after a partial legacy round is removed.
